...
double computeWavelengthInCm(double dFrequencyHz)
{
  ...
  if ((dFrequencyHz < 0.0) || (!isfinite(dFrequencyHz)) || (FP_ZERO == fpclassify(dFrequencyHz)))
  {
    ...
    // /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/ //
    dWavelengthCm = k_dSpeedOfSoundMetersPerSecond / dFrequencyHz;
  }
...
int main()
{
  ...
  for (int nu = 0; nu < nOctaves; nu++)
  {
    for (int k = 0; k < k_nHalfTonesInOctave; k++)
    {
      ...
      double f = computeFrequency(nu, k_nHalfTonesInOctave);
      double dWavelengthCentimeters = computeWavelengthInCm(f);

      string strNote = getNoteName(nu, k_nHalfTonesInOctave);
