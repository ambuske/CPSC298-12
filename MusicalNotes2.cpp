...
int main(){
  ...
  double f = 0;
  double dWavelengthCentimeters = 0;
...
//  const int k_nOctaves = 9; // Number of octaves to consider
  ...
  for (int nu = 0; nu<=8; nu++){
    for (int k = 0; k<=11; k++){
      f = ref*pow(2, nu+(k/12.0));
      dWavelengthCentimeters = speedofSound/f;
      dWavelengthCentimeters *= 100;
      offset = k_nHalfTones * nu +k;
  ...
  // ?? (Need to compute offset into array based on nu and k)
      ...
      << dWavelengthCentimeters << " cm" << endl;
    }
  }
}
